    ...
    switch (dir) {
      case 'races':
        ...
        if (fc.attributes.traits) {
          for (const trait of fc.attributes.traits) {
        ...
        if (fc.attributes.variants) {
          for (const subrace of fc.attributes.variants) {
      ...
      case 'vehicles':
        ...
        if (fc.attributes.weapons) {
          for (const attack of fc.attributes.weapons) {
  ...
  for (const thing of things) {
    ...
    switch (file) {
      ...
      case 'bestiary':
        ...
        item.body = ''
        for (const key of ['Actions', 'Features', 'Reactions', 'Lair Actions', 'Legendary Actions']) {
          const splitKey = key.split(' ')
          const attrKey = `${splitKey[0].toLowerCase()}${splitKey[1] || ''}`
          if (thing[attrKey].length > 0) {
            let groupString = ` ${key}`
            for (const v of thing[attrKey]) {
              let string = ` ${v.name}`
              switch (v.type) {
                case 'weapon':
                case 'grenade':
...
for (const klass of classes) {
  ...
  for (const p of klass.progression) {
    for (const f of p.features) {
      if (f === 'ability_score_improvement') {
        continue
      }
      if (f === 'subclass') {
        for (const sc of klass.subclasses) {
          for (const scf of sc.features[subclassFeatureIndex]) {
