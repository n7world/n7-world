<template lang="pug">
  v-app(toolbar)
    ...
    // Jump Links
    v-navigation-drawer(v-model="rulebarVisible" fixed right clipped app width="225" mobile-breakpoint="960" v-if="page.rules")
      v-list(dense)
        v-subheader Jump to
        v-list-item(ripple v-for="(rule, index) in rules" v-bind:key="index" v-on:click="goToRule(`#${rule.hash}`)")
          v-list-item-content
            v-list-item-title {{ rule.title }}
    ...
    // Content
    v-main
      h1.sr-only N7 World - Player's Manual
      nuxt
</template>
