...
export default {
  ...
  methods: {
    ...
    hasPowers (power_arr) {
      const c_id = this.character.class.id
      const arr = power_arr
    ...
    calcAbilityMod (ability_score) {
      return Math.floor((ability_score - 10) / 2)
    },
    calcAbilitySavingThrow (ability_score, isProf) {
      let num = this.calcAbilityMod(ability_score)
      if (isProf) {
    ...
    calcSkillMod (ability_str, prof_score) {
      let baseMod = this.calcAbilityMod(this.character.stats[ability_str])
      if (prof_score > 0) {
        baseMod += Math.floor(prof_score * this.calcProfBonus(this.character.level))
      }
      return baseMod
    },
    calcSpellDC (ability_score) {
